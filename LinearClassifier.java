...
abstract public class LinearClassifier {
	...
	private ArrayList<Double> trainingReport = new ArrayList(); //direct indexed arrayList to the percentage from training report
	 ...
	 * See AIMA p. 724.
	 */
	public void train(List<Example> examples, int nsteps) {
		Random random = new Random();
		int n = examples.size();
		for (int i=1; i <= nsteps; i++) {
			int j = random.nextInt(n);
			Example ex = examples.get(j);
			this.update(ex.inputs, ex.output, 1000.0/(1000.0+i));
	...
	protected void trainingReport(List<Example> examples, int stepnum, int nsteps) {
		...
		trainingReport.add(accuracy(examples));
	}
	
	/**
	 * Return the squared error per example for this LinearClassifier
